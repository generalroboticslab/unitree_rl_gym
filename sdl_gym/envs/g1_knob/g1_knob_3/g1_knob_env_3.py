from sdl_gym.envs.g1_knob.g1_knob_3.g1_robot_3 import G1Robot
...
class G1KnobRobot_3(G1Robot):
    ...
    # def transform_jacobian_to_global(sefl, robot, q, J_local, end='right_wrist_yaw_link', start='right_shoulder_pitch_link', base='torso_link'):
    #     """
    #     将局部坐标系的 Jacobian 转换到全局坐标系
        
    #     参数：
    #     - robot: roboticstoolbox 机器人对象
    #     - q: 机器人关节角度
    #     - J_local: 局部坐标系的 Jacobian 矩阵
    #     - end: 计算 Jacobian 的终端 link
    #     - start: 局部 Jacobian 所基于的 link
    #     - base: 目标全局坐标系的基准 link
        
    #     返回：
    #     - J_global: 变换到全局坐标系的 Jacobian
    #     """
    #     # 计算 torso_link 到 right_shoulder_pitch_link 的变换矩阵
    #     T_base_to_shoulder = robot.fkine(q, end=start, start=base)  # 计算齐次变换
    #     R_base_to_shoulder = T_base_to_shoulder.R  # 提取旋转矩阵 (3x3)

    #     # 构造 Adjoint 变换矩阵 (6x6)
    #     Ad_T = np.block([
    #         [R_base_to_shoulder, np.zeros((3,3))],
    #         [np.zeros((3,3)), R_base_to_shoulder]
    #     ])

    #     # 变换 Jacobian
    #     J_global = Ad_T @ J_local

    #     return J_global

    def transform_jacobian_to_global(self, J_local, q):
        """
        转换局部坐标系的Jacobian到全局坐标系
        :param J_local: (6, num_dofs) 的Jacobian，相对于base link坐标系
        :param R_base: (3, 3) 的旋转矩阵，base link相对于全局坐标系
        :return: (6, num_dofs) 的全局坐标系Jacobian
        """
        from scipy.spatial.transform import Rotation as R
        J_v_local = J_local[:3, :]  # 取出线速度部分
        J_w_local = J_local[3:, :]  # 取出角速度部分
        
        r = R.from_quat(q)  # Scipy 处理的是 [qx, qy, qz, qw] 格式
        R_base = r.as_matrix()
        
        ######### NOT SURE #########
        R_base = R_base.T

        J_v_global = R_base @ J_v_local
        J_w_global = R_base @ J_w_local

        J_global = np.vstack([J_v_global, J_w_global])
        return J_global
    ...
    def _process_jacobian(self):
        ...
        for i in range(self.num_envs):
            ...
            J_local = self.ik_helper_robot.jacobe(
                q=self.ik_helper_robot.q, 
                end='right_wrist_yaw_link', 
                start='right_shoulder_pitch_link'
            )
            
            # J_global = self.transform_jacobian_to_global(
            #     self.ik_helper_robot, self.ik_helper_robot.q, J_local, 
            #     end='right_wrist_yaw_link', start='right_shoulder_pitch_link', base='torso_link'
            # )
            
            # J_global = self.transform_jacobian_to_global(J_local, self.rigid_body_states[i, self.torso_link_index, 3:7].cpu().numpy())
            
            # import ipdb; ipdb.set_trace()
             
            self.right_hand_jacobian[i, :] = torch.tensor(J_local, device=self.device)
